...
export default async function DestinationsPage({ searchParams }: Props) {
  ...
  return (
    <div className="flex flex-col flex-1 lg:h-[calc(100dvh-4rem)] lg:max-h-[calc(100dvh-4rem)] lg:overflow-hidden bg-gray-50">
      <header className="relative shrink-0 bg-navy-900 overflow-hidden">
      ...
      <div className="shrink-0 max-w-6xl w-full mx-auto px-6 pt-4 pb-3">
      ...
      <div className="flex-1 min-h-0 max-w-6xl w-full mx-auto px-6 pb-4 md:pb-6">
        {destinations.length === 0 ? (
          <p className="text-gray-500 text-center py-12">
            {t(locale, "noDestinationsInCategory")}
          </p>
        ) : (
          <div className="h-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5 lg:auto-rows-fr">
            {destinations.map((destination) => (
              <Link
                key={destination.id}
                href={`/destinations/${destination.slug}`}
                className="card group flex flex-col min-h-[220px] lg:min-h-0 lg:h-full"
              >
                <div className="relative flex-1 min-h-[140px] lg:min-h-0 overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 pt-10">
                    <h2 className="font-bold text-white text-lg leading-tight group-hover:text-wandr-200 transition-colors">
                      {destination.name}
                    </h2>
                    <p className="text-sm text-white/75 mt-1 line-clamp-2 leading-snug">
                      {destination.tagline}
                    </p>
                  </div>
                </div>
